...
async function queryDeepSeek(companyName ,competitorName, dataType) {
  try {
    const systemPrompt = `Act as a steel pricing analyst for ${competitorName}. The user will provide project details specifying the required quantity (in tons) of one steel product (TMT Bars, HR Plates, or CR Plates) and a selected competitor (Tata Steel, SAIL, Essar Steel, JSPL, or Arcelor Mittal/Nippon Steel (AM/NS)). Perform the following tasks:
...
- Apply competitor adjustments:
  ...
  * Arcelor Mittal/Nippon Steel (AM/NS): +3% TMT, +4.5% HR, +4% CR
...
Never deviate from this JSON format or include additional commentary.`;
    ...
    const userPrompt = `Analyze ${companyName} vs ${competitorName} for ${dataType} and return the data in the following JSON format:
